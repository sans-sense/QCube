describe("QueryEngine", function() {
    ...
    function getResultValue(resultWrapper) {
        return resultWrapper[0].value;
    }
    ...
    it('for *,*,*  gives a sum of all table data i.e 27, root works', function(){
        ...
        expect(getResultValue(QE.find(this.qcTree, ['*', '*', '*']))).toEqual(total);
    });

    it('for S1, P1, *  gives sum of all records with S1 and P1 i.e. 6, searchRoute last child logic' , function() {
        expect(getResultValue(QE.find(this.qcTree, ['S1', 'P1', '*']))).toEqual(6);
    });

    it('for [S1, *, *]  gives sum of all records with S1  i.e. 18, pick last dim child logic' , function() {
        expect(getResultValue(QE.find(this.qcTree, ['S1', '*', '*']))).toEqual(18);
    });

    it('for impossible combinations like [S1, P1, f] should give null as there are no records with S1, P1 and f' , function() {
        expect(getResultValue(QE.find(this.qcTree, ['S1', 'P1', 'f']))).toEqual(null);
    });

    it('for [[S1, S2, S3], [P1, P3], s] gives 6', function(){
        expect(QE.find(this.qcTree, [['S1','S2','S3'], ['P1','P3'], 's'])).toEqual([{ key:['S1','P1','s'], value:6 }]);
    });

    it('for [[S1, S2, S3], [P1, P2], s] gives 6 and 12', function(){
        expect(QE.find(this.qcTree, [['S1','S2','S3'], ['P1','P2'], 's'])).toEqual([{key:['S1','P1','s'], value:6}, { key:['S1','P2','s'], value:12}]);
    });

    it('for [[S1, S2, S3], [P1, P2], *] gives 6, 12 and 9', function(){
        expect(QE.find(this.qcTree, [['S1','S2','S3'], ['P1','P2'], '*'])).toEqual([{key:['S1','P1','s'], value:6}, { key:['S1','P2','s'], value:12}, { key:['S2','P1','f'], value:9} ]);
    });

    it('for [*, [P1,P2],*] gives 15 and 12', function(){
        expect(QE.find(this.qcTree, ['*', ['P1','P2'], '*'])).toEqual([ { key:['*','P1','*'], value: 15 }, { key:['S1','P2','s'],value: 12 }]);
    });
